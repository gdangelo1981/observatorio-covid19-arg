...
def loadData(fileName, columnName): 
    ...
    data['Province/State'].fillna('<Todos>', inplace=True)
    data[columnName].fillna(0, inplace=True)
    return data

allData = loadData("time_series_covid19_confirmed_global.csv", "CumConfirmados") \
    .merge(loadData("time_series_covid19_deaths_global.csv", "CumMuertos")) \
    .merge(loadData("time_series_covid19_recovered_global.csv", "CumRecuperados"))
...
app.layout = html.Div(
    ...
    children=[
	html.Title('Observatorio'),
    html.H1('Observatorio Coronavirus (COVID-19) - by 54cuatro/Shambala/Zaffir'),
    html.Div(className="row", children=[
        html.Div(className="four columns", children=[
            html.H5('Seleccione el Pais'),
            dcc.Dropdown(
                id='country',
        ...
        html.Div(className="four columns", children=[
            html.H5('Seleccione Metricas'),
            dcc.Checklist(
                id='metrics',
                options=[{'label':m, 'value':m} for m in ['Confirmados', 'Muertos', 'Recuperados']],
                value=['Confirmados', 'Muertos', 'Recuperados']
            )
        ])
    ]),
    dcc.Graph(
...
def barchart(data, metrics, prefix="", yaxisTitle=""):
    figure = go.Figure(data=[
        go.Bar( 
            ...
            marker_color={ 'Muertos':'rgb(0,0,0)', 'Recuperados':'rgb(30,200,30)', 'Confirmados':'rgb(100,140,240)'}[metric]
...
def update_plot_new_metrics(country, state, metrics):
    ...
    return barchart(data, metrics, prefix="New", yaxisTitle="Nuevos casos diarios")
...
def update_plot_cum_metrics(country, state, metrics):
    data = nonreactive_data(country, state)
    return barchart(data, metrics, prefix="Cum", yaxisTitle="Historico")
